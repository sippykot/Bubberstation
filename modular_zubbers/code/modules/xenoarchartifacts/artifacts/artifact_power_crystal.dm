...
/obj/machinery/power/crystal
	name = "large crystal"
	icon = 'modular_zubbers/code/modules/xenoarchartifacts/icons/artifacts.dmi'
...
/obj/machinery/power/crystal/proc/update_crystal()
	if(wired && anchored && first_effect.activated)
		...
		icon_state = icon_custom_crystal
	if(wired)
		add_overlay(image('modular_zubbers/code/modules/xenoarchartifacts/icons/artifacts.dmi', "crystal_overlay"))
	else
		cut_overlays()
